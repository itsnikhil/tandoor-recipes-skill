...
async function searchRecipes(query: string, limit: number = 10): Promise<void> {
    const data = await apiRequest(
        `/api/recipe/?query=${encodeURIComponent(query)}&page_size=${limit}`,
        RecipeListResponseSchema
    );
...
async function getMealTypeId(name: string): Promise<number | null> {
    const types = await apiRequest('/api/meal-type/', z.array(MealTypeSchema));
...
async function getMealPlans(fromDate: string, toDate?: string): Promise<void> {
...
async function getShoppingList(checked: string = 'false'): Promise<void> {
    const items = await apiRequest(
        `/api/shopping-list-entry/?checked=${checked}`,
        ShoppingListResponseSchema
    );
...
async function lookupFoodId(name: string): Promise<{ id: number; name: string } | null> {
    const data = await apiRequest(
        `/api/food/?query=${encodeURIComponent(name)}`,
        FoodListResponseSchema
    );
    return data.results[0] ?? null;
}

async function lookupUnitId(name: string): Promise<{ id: number; name: string } | null> {
    const data = await apiRequest(
        `/api/unit/?query=${encodeURIComponent(name)}`,
        UnitListResponseSchema
    );
    return data.results[0] ?? null;
}
...
async function checkShoppingItem(itemId: number): Promise<void> {
...
async function removeShoppingItem(itemId: number): Promise<void> {
...
async function createRecipe(
    ...
    const result = await apiRequestRaw('/api/recipe/', { method: 'POST', body: payload }) as { id: number };
    console.log(`Created recipe "${name}" with ID: ${result.id}`);
}

async function getKeywords(query?: string): Promise<void> {
...
async function getFoods(query?: string): Promise<void> {
...
async function getUnits(query?: string): Promise<void> {
